...
export default async function CoinMarketSSR() {
    ...
    const res = await fetch(`https://api.coinranking.com/v2/coin${API_KEY}`, 
      {cache: "no-store",}
    );
    ...
    return (
      ...
      <h1 className=" text-center text-4xl text-bold mb-5 font-serif ">Coin SSR</h1>
      <div className=" border rounded-lg min-w-fit  border-white p-1 sm:p-8 lg:p-8 md:p-8 ">
      <div className="grid sm:grid-cols-2 grid-cols-1 items-center gap-4 ">
      
      <button className="btn btn-wide border-2 border-2-gray-400">
         BNB: {data?.rates?.BNB || "Loading..."}
        </button>
      
      <button className="btn btn-wide  border-2-gray-400">
